          ...
          {!result ? (
            ...
            <div className="space-y-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="rounded-[18px] border border-borderGlass/12 bg-white/55 px-4 py-3 text-right">
              ...
              <div className="grid gap-3 md:grid-cols-3">
                <div className="rounded-[20px] border border-borderGlass/10 bg-white/55 px-4 py-3">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="rounded-[20px] border border-borderGlass/10 bg-white/55 p-4">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {result.detected_families.length ? (
                      result.detected_families.map((family) => (
                        <span
                          key={family}
                          className="rounded-full border border-borderGlass/12 bg-panelSoft/70 px-3 py-2 text-xs uppercase tracking-[0.18em] text-ink"
                        >
                          {titleCase(family)}
                        </span>
                      ))
                    ...
                    {result.recommended_playbooks.length ? (
                      result.recommended_playbooks.map((playbook) => (
                        <div
                          ...
                          className="rounded-[16px] border border-borderGlass/10 bg-panelSoft/75 px-3 py-3"
                        >
              ...
              <div className="rounded-[20px] border border-borderGlass/10 bg-panelSoft/80 p-4">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {result.signals.map((signal) => (
                    <span
                      key={signal}
                      className="rounded-full border border-borderGlass/12 bg-white/55 px-3 py-1.5 text-[0.68rem] uppercase tracking-[0.16em] text-muted"
                    >
                      {signal}
                    </span>
                  ))}
